...
namespace AB
{
    public partial class SelectedItem : Form
    {
        ...
        public void loadData()
        {
            if (dtSelectedItems.Rows.Count > 0)
            {
                ...
                foreach (DataRow row in dtSelectedItems.Rows)
                {
                    ...
                    dgv.Rows.Add(row["item_code"].ToString(), String.Format("{0:#,0.000}", quantity), row["uom"].ToString(), row["whsecode"].ToString());
                }
            }
        }
        ...
        private void btnBack_Click(object sender, EventArgs e)
        {
            ...
            dtDgv.Columns.Add("whsecode");
            dtDgv.Rows.Clear();
            for(int i =0; i < dgv.Rows.Count; i++)
            {
                string itemName = dgv.Rows[i].Cells["item_code"].Value.ToString();
                double quantity = double.TryParse(dgv.Rows[i].Cells["quantity"].Value.ToString(), out doubleTemp) ? Convert.ToDouble(dgv.Rows[i].Cells["quantity"].Value.ToString()) : doubleTemp;
                string uom = dgv.Rows[i].Cells["uom"].Value.ToString();
                string whseCode = dgv.Rows[i].Cells["whsecode"].Value.ToString();
                dtDgv.Rows.Add(itemName, quantity.ToString(), uom,whseCode);
            }
                ...
                if (SAP_Remarks.isSubmit)
                {
                    ...
                    if (Login.jsonResult != null)
                    {
                        ...
                        if (!token.Equals(""))
                        {
                            ...
                            jObjectHeader.Add("transdate", DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss"));
                            if (sap_number <= 0)
                            {
                            ...
                            if(dgv.Rows.Count > 0)
                            {
                                for (int i = 0; i < dgv.Rows.Count; i++)
                                {
                                    ...
                                    jObjectRows.Add("whsecode", whseCode);

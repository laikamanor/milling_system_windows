...
namespace AB
{
    public partial class Receive_ForSAPDetails : Form
    {
        public Receive_ForSAPDetails(string sSelectedIds)
        {
        ...
        private void Receive_ForSAPDetails_Load(object sender, EventArgs e)
        {
            buttonUpdateSAP.Visible = !hasSAP;
        ...
        {
            string sResult = apic.loadData("/api/inv/recv/for_sap/details", "?ids=%5B" + selectedIds + "%5D", "", "", RestSharp.Method.GET, true);
        ...
        private void buttonUpdateSAP_Click(object sender, EventArgs e)
        {
            if (!hasSAP)
            {
                ...
                if (SAP_Remarks.isSubmit)
                {
                    ...
                    string sResult = apic.loadData("/api/inv/recv/update_sap", "", "application/json", joData.ToString(), RestSharp.Method.PUT, true);

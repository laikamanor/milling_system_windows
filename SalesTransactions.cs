...
namespace AB
{
    public partial class SalesTransactions : Form
    {
        ...
        int cDocStatus = 1, cBranch = 1, cWarehouse = 1, cFromDate = 1, cToDate = 1, cCustType = 1;
        ...
        private void dtFromDate_CloseUp_1(object sender, EventArgs e)
        {
            if (cFromDate <= 0)
            {
                loadData();
        ...
        public async void loadData()
        {
            ...
            if (Login.jsonResult != null)
            {
                ...
                if (!token.Equals(""))
                {
                    ...
                    string sTransdate = "&from_date=" + dtFromDate.Value.ToString("yyyy-MM-dd") + "&to_date=" + dtToDate.Value.ToString("yyyy-MM-dd");

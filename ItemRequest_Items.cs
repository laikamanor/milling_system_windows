...
namespace AB
{
    public partial class ItemRequest_Items : Form
    {
        ...
        private void ItemRequest_Items_Load(object sender, EventArgs e)
        {
            ...
            loadData();
            btnCloseTransaction.Visible = forType.Equals("Open");
            btnPrint.Location = btnCloseTransaction.Visible ? btnPrint.Location : btnCloseTransaction.Location;
            //btnCloseTransaction.BackColor = forType.Equals("For Confirmation") ?Color.ForestGreen : Color.DodgerBlue;
            //btnCloseTransaction.Text = forType.Equals("For Confirmation") ? "Confirm" : "Update SAP #";
            //btnCancel.Visible = forType.Equals("For Confirmation") ? true : false;
            //btnCloseTransaction.Visible = forType.Equals("Logs") ? false : true;
        }
        ...
        public void loadData()
        {
            ...
            if (Login.jsonResult != null)
            {
                ...
                if (!token.Equals(""))
                {
                    ...
                    Console.WriteLine("item request details " + response.Content);
                        ...
                        foreach (var z in jObjectResponse)
                        {
                            if (z.Key.Equals("data"))
                            {
                                if (z.Value.ToString() != "{}")
                                {
                                    ...
                                    foreach (var y in jObjectData)
                                    {
                                        if (y.Key.Equals("request_rows"))
                                        {
                                            if (y.Value.ToString() != "[]")
                                            {
                                                ...
                                                {
                                                    int itemID = 0, intTemp = 0;
                                                    string itemName = "";
                                                    double quantity = 0.00, delQty = 0.00, doubleTemp = 0.00;
                                                    ...
                                                    foreach (var w in jObjectRequestRows)
                                                    {
                                                        ...
                                                        if (w.Key.Equals("id"))
                                                        {
                                                            itemID = int.TryParse(w.Value.ToString(), out intTemp) ? Convert.ToInt32(w.Value.ToString()) : intTemp;
                                                        }
                                                        ...
                                                        {
                                                            quantity = double.TryParse(w.Value.ToString(), out doubleTemp) ? Convert.ToDouble(w.Value.ToString()) : doubleTemp;
                                                        }
                                                        else if (w.Key.Equals("deliverqty"))
                                                        {
                                                            delQty = double.TryParse(w.Value.ToString(), out doubleTemp) ? Convert.ToDouble(w.Value.ToString()) : doubleTemp;
                                                        }
                                                    }
                                                    dgv.Rows.Add(itemID, itemName, quantity,delQty);
                                                }
                                            }
                                        }
                                    ...
                                    lblDueDate.Text = dtTransDate.ToString("yyyy-MM-dd HH:mm:ss");
                                    lblRequestDate.Text = dtCreated.ToString("yyyy-MM-dd HH:mm:ss");
                                }   
                            }
                        }
                    }
        ...
        public void forManualClosed()
        {
            ...
            {
                string URL = "/api/inv/item_request/manual_close/" + selectedID;
                Remarks remarkss = new Remarks();
                remarkss.ShowDialog();
                if (Remarks.isSubmit)
                {

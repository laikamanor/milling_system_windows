...
namespace AB
{
    public partial class Login : Form
    {
        ...
        public static string fullName = "", selectedShift = "";
        public Login()
        {
            InitializeComponent();
        }
        ...
        {
            var strContent = utilityc.getTextfromGithub(utilityc.abWindowsVersionFile).Trim();
            //double tempDouble = 0;
            ...
            //MessageBox.Show(strContent + Environment.NewLine + utilityc.versionName);
            Console.WriteLine(strContent + "/" + utilityc.versionName.Trim());
            if (strContent.Trim() != utilityc.versionName.Trim())
            {
                ...
                {
                    //if (!File.Exists(@"D:\esystem_update_files"))
                    //{
                    //    // Create a app_path text file     
                    //    using (StreamWriter sw = File.CreateText(@"D:\esystem_update_files\app_path.txt"))
                    //    {
                    ...
                    //else
                    //{
                    //    System.IO.File.WriteAllText(@"D:\esystem_update_files.txt", Application.StartupPath);
                ...
                if (response.ErrorMessage == null)
                {
                    if (response.Content.Substring(0, 1).Equals("{"))
                    {
                        ...
                        if (isSuccess)
                        {
                            ...
                            ViewAssignedDept frm2 = new ViewAssignedDept(assignDept.ToString(), userName, true);
                            ...
                            if (ViewAssignedDept.isSubmit)
                            {
                                ...
                                if (!string.IsNullOrEmpty(sResult.Trim()))
                                {
                                    if (sResult.StartsWith("{"))
                                    {
                                        ...
                                        bool isSuccess2 = false, boolTemp = false;
                                        isSuccess2 = joResult["success"] == null ? boolTemp : bool.TryParse(joResult["success"].ToString(), out boolTemp) ? Convert.ToBoolean(joResult["success"].ToString()) : boolTemp;
                                        string msg2 = joResult["message"] == null ? "" : joResult["message"].ToString();
                                        if (isSuccess2)
                                        {
                                            string newDepartment = joResult["data"]["branch"] == null ? "" : joResult["data"]["branch"].ToString();
                                            Login.jsonResult["data"]["branch"] = newDepartment;
                                            //apic.showCustomMsgBox(isSuccess2 ? "Message" : "Validation", msg2);
                                            ...
                                            {
                                                selectedShift = showWarehouse.selectedWhse;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                var client = new RestClient("http://localhost:58546");
                client.AddDefaultHeader("Content-Type", "application/json");
                client.Timeout = -1;
                var request = new RestRequest("/api/weather");
                var response = client.Execute(request);
                if (response.ErrorMessage == null)
                {

        ...
        public async Task <DataTable> returnWarehouse(string branch,string appendParams)
        {
            ...
            if (Login.jsonResult != null)
            {
                ...
                if (!token.Equals(""))
                {
                    ...
                    var request = new RestRequest("/api/whse/get_all" + (string.IsNullOrEmpty(branch) ? "?branch=" : "?branch=" + branch) + appendParams);
                    ...
                    else if (response.Content.ToString().Substring(0, 1).Equals("{"))
                    {
